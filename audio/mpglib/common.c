...
head_check(unsigned long head, int check_layer)
{
    ...
    /* lock layer=3, we only work for mp3, if not,
     * will big chance match a error frame head, then cause halt.
     * The better practice is keep mp->fr->lay at decode_reset before seeking.
     * add by jzy. */
    check_layer = 3;
    /* bits 13-14 = layer 3 */
    int     nLayer = 4 - ((head >> 17) & 3);
    int undefVer = ((head>>19) & 3);  //undefined version
